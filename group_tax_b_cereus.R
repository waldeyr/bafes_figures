...
# get the selected group (B. cereus like)
group_tax_b_cereus <- read_delim(paste0(here(),"/group_tax_b_cereus.tsv"), "\t", escape_double = FALSE, trim_ws = TRUE)

# Filtering the results for the selected group 
b_cereus_filtered <- response_to_biochemical_and_physiological_tests %>%
  filter(as.numeric(SDF) %in% as.numeric(group_tax_b_cereus$SDF))

# concatenating the species names and their isolates id (SDF9999)
rownames <- as.character(paste0(b_cereus_filtered$Taxonomy, " (SDF", b_cereus_filtered$SDF, ")"))
...
b_cereus_filtered <- dplyr::select(b_cereus_filtered, -c(NCBI, SDF)) 
View(b_cereus_filtered)

# Converting the dataset into a numeric matrix omitting the first (string) column
matrix <- data.matrix(b_cereus_filtered[,-1])
# View(matrix)
