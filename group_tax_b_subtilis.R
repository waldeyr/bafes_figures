...
# get the selected group (B. subtilis like)
group_tax_b_subtilis <- read_delim(paste0(here(),"/group_tax_b_subtilis.tsv"), "\t", escape_double = FALSE, trim_ws = TRUE)

# Filtering the results for the selected group 
b_subtilis_filtered <- response_to_biochemical_and_physiological_tests %>%
  filter(as.numeric(SDF) %in% as.numeric(group_tax_b_subtilis$SDF))

# concatenating the species names and their isolates id (SDF9999)
rownames <- as.character(paste0(b_subtilis_filtered$Taxonomy, " (SDF", b_subtilis_filtered$SDF, ")"))
...
b_subtilis_filtered <- dplyr::select(b_subtilis_filtered, -c(NCBI, SDF)) 
# View(B_subtilis_filtered)

# Converting the dataset into a numeric matrix omitting the first (string) column
matrix <- data.matrix(b_subtilis_filtered[,-1])
# View(matrix)
